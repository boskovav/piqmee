...
//    private double [] storedCurrentCategoryRates;
    ...
    public void initAndValidate() {
        ...
        logProbabilities = new double[m_nStateCount * siteModel.getCategoryCount()];

        rates = new double[m_nStateCount];
    ...
    private boolean initialize() {
        ...
        int leafNodeCount = m_nNodeCount / 2 + 1;
        ...
        m_branchLengths = new double[m_nNodeCount + leafNodeCount];
        storedBranchLengths = new double[m_nNodeCount + leafNodeCount];
...
// System.err.println("BEAGLE :" + logL);        
        logP = logL;
        return logL;
    }
...
//        }
        ...
        if (node.isLeaf()) {
            ...
            toyNode.setNr(nodeCount + nodeNum);
        	...
        	if  (update != Tree.IS_CLEAN  || totalBranchTime != m_branchLengths[m_nNodeCount + nodeNum]) {
	        	
	        	m_branchLengths[m_nNodeCount + nodeNum] = totalBranchTime;
	        	...
	        	calculateLogLeafScale(nodeNum, logProbabilities);
        	}
        }
        
        
        
        if (!node.isRoot() && (update != Tree.IS_CLEAN || Math.abs(branchTime - m_branchLengths[nodeNum]) > 0)) {
