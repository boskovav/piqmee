...
public class QuasiSpeciesSiteModel extends QuasiSpeciesSiteModelInterface.Base {
    ...
    @Override
    public int getCategoryOfSite(final int site, final QuasiSpeciesNode node) {
        throw new IllegalArgumentException("Integrating across categories");
    }

    @Override
    public double getRateForCategory(final int category, final QuasiSpeciesNode node) {
        synchronized (this) {
            if (!ratesKnown) {
     ...
     */
    @Override
    public double[] getCategoryRates(final QuasiSpeciesNode node) {
        synchronized (this) {
            if (!ratesKnown) {
                calculateCategoryRates(node);
            }
     ...
     */
    @Override
    public double getProportionForCategory(final int category, final QuasiSpeciesNode node) {
        synchronized (this) {
            if (!ratesKnown) {
                calculateCategoryRates(node);
            }
     ...
     */
    @Override
    public double[] getCategoryProportions(final QuasiSpeciesNode node) {
        synchronized (this) {
            if (!ratesKnown) {
                calculateCategoryRates(node);
            }
