...
//
     ...
     /**
     * QS OWN FUNCTION
     * Calculates partial likelihood at a node.
     */
    protected void calculateStatesPruning(int[] iStates, double[] fMatrices,
                                                double[] fPartials3) {

        Arrays.fill(fPartials3, 0);
...
//                    for (int i = 0; i < nrOfStates; i++) {
                        ...
                        fPartials3[v+state] = fMatrices[w + state*(nrOfStates+1)];
...
//    }
     ...
     * @param fOutLogLikelihoods an array into which the likelihoods will go
     */
    public void calculateQSLogLikelihoods(double[] fPartials,double[] fOutLogLikelihoods) {
        int v = 0;
        for (int k = 0; k < nrOfPatterns; k++) {
            double sum = 0.0;
            for (int i = 0; i < nrOfStates; i++) {
...
//        }
     ...
     * Calculates partial likelihoods at a node.
     *
     * @param iNodeIndex the QS node with haplotype start time array information (unique tip in the tree)
     */
    public void calculatePartials(int iNodeIndex) {
        calculateStatesPruning(states[iNodeIndex], matrices[currentMatrixIndex[iNodeIndex]][iNodeIndex],
                partials[currentPartialsIndex[iNodeIndex]][iNodeIndex]);

        if (useScaling) {
            scalePartials(iNodeIndex);
        }
    }
